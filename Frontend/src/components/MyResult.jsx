      ...
      try {
        const q =
          tech && tech.toLowerCase() !== "all"
            ...
            : "";
        const res = await axios.get(`${apiBase}/api/results/list${q}`, {
...
// of available `technologies` for filter buttons.
  useEffect(() => {
    ...
    const fetchAllForTechList = async () => {
      try {
        const res = await axios.get(`${apiBase}/api/results/list`, {
          headers: { "Content-Type": "application/json", ...getAuthHeader() },
          timeout: 10000,
        });
        if (!mounted) return;
  ...
  return source.reduce((acc, item) => {
    const track = item.technology || "General";
      ...
      <div className={resultStyles.filterContainer}>
        <div className={resultStyles.filterContent}>
         <div  className={resultStyles.filterButtons}>
           ...
           selectedTechnology==='all' ? resultStyles.filterButtonActive : resultStyles.filterButtonInactive
    ...
    <div className={resultStyles.resultsGrid}>
      {items.map((r) => (
        ...
        {Array.isArray(results) && results.length===0 && !error &&(
          ...
          No results yet.Take a quix to see your result here
          </div>
        )}
      </>
    )}
...
function StripCard({ item }) {
  ...
  return (
    <article className={resultStyles.card}>
      <div className={resultStyles.cardAccent}>
